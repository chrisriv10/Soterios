...
window.Pages.optimization = {
  async render(container) {
    ...
    const modeInfo = {
      balanced: { icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m16 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="m2 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"/></svg>', tip: 'Best for everyday work, browsing, and mixed workloads.' },
      gaming: { icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="6" x2="10" y1="11" y2="11"/><line x1="8" x2="8" y1="9" y2="13"/><line x1="15" x2="15.01" y1="12" y2="12"/><line x1="18" x2="18.01" y1="10" y2="10"/><path d="M17.32 5H6.68a4 4 0 0 0-3.978 3.59c-.006.052-.01.101-.017.152C2.604 9.416 2 14.456 2 16a3 3 0 0 0 3 3c1 0 1.5-.5 2-1l1.414-1.414A2 2 0 0 1 9.828 16h4.344a2 2 0 0 1 1.414.586L17 18c.5.5 1 1 2 1a3 3 0 0 0 3-3c0-1.545-.604-6.584-.685-7.258-.007-.05-.011-.1-.017-.151A4 4 0 0 0 17.32 5z"/></svg>', tip: 'Prioritizes responsiveness and sustained performance; it may use more power.' },
      quiet: { icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>', tip: 'Reduces power use and fan noise; demanding work may run more slowly.' }
    };
    container.innerHTML = `<section class="optimization-page">
      ...
      <div class="card optimization-status-card"><div><div class="panel-title">Current power plan</div><div id="optimizationCurrent" class="optimization-current">${escapeHtml(t('common.loading'))}</div><p id="optimizationStatus" class="page-subtitle"></p></div></div>
      <div class="optimization-grid" id="optimizationGrid"><div class="empty-state">${escapeHtml(t('common.loading'))}</div></div>
      <div class="card optimization-tips"><div class="panel-title">Choose with confidence</div><p>Changing a plan is reversible and applies immediately. Soterios only changes the active Windows power plan; it does not overclock hardware or modify BIOS settings.</p><ul><li>Use <strong>Balanced</strong> for normal daily use.</li><li>Use <strong>Gaming / Performance</strong> for sustained CPU or GPU workloads.</li><li>Use <strong>Quiet / Battery Saver</strong> when battery life and low fan noise matter most.</li></ul></div>
    </section>`;
    ...
    grid.addEventListener('click', async (event) => {
      ...
      try {
        ...
        if (!result?.ok) throw new Error(result?.error || t('settings.performanceMode.loadError'));
        active = busy;
        status.textContent = `Applied ${t(`settings.performanceMode.${active}.name`)}.`;
      } catch (error) {
