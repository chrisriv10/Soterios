(function () {
  ...
  const ToolsPage = {
    ...
    _renderControls(tool, run) {
      ...
      switch (tool.id) {
        ...
        case 'maintenance-safety-vault':
          return `<div class="maintenance-controls"><button type="button" class="btn btn-ghost" data-action="refresh-vault">${this.e(this.t('tools.vault.refresh', 'Refresh Vault'))}</button><span>${this.e(this.t('tools.vault.expiryNote', 'Items expire seven days after they are staged.'))}</span></div>`;
    ...
    async _handleAction(target) {
      ...
      if (action === 'delete-vault-log') { await this._deleteVaultLog(target.dataset.vaultId); return; }
    ...
    async _deleteVaultLog(id) {
      const ok = await this._confirm({ title: 'Remove this Vault log?', message: 'This removes only the File Vault history entry. It does not change the restored file or recover any deleted data.', confirmLabel: 'Remove log', danger: true });
      if (!ok) return;
      const response = await window.soterios.vault.deleteLog(id);
      if (!response.ok) throw new Error(response.error);
      this._setNotice('Vault log removed.', 'success');
      await this._loadServiceState('maintenance-safety-vault'); this._renderView();
    },
