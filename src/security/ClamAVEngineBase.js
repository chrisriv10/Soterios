...
class ClamAVEngineBase {
  ...
  async scanFile(filePath, onProgress) {
    ...
    return new Promise((resolve) => {
      const args = this._spawnArgs(this.clamscanPath, filePath, isDir);
  ...
  abortCurrentScan() {
    ...
    if (this.activeScanProcess) {
      ...
      try {
        ...
        logger.debug('activeScanProcess.kill failed', { error: e?.message || String(e) });
      }
    }
    ...
    if (this.activeUpdateProcess) {
      ...
      try {
        ...
        logger.debug('activeUpdateProcess.kill failed', { error: e?.message || String(e) });
      }
    }
...
module.exports = ClamAVEngineBase;
