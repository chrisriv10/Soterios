...
| `compile-filter-lists.js` | `npm run filter:compile` — compiles vendored EasyList/EasyPrivacy snapshots (`filter-lists/`) into MV3 DNR rulesets (`browser-extension/rules/`); `--check` verifies checked-in artifacts without writing, `--update` re-fetches pinned snapshots |
